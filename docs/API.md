...
| `goss_engine_capture_still` | `captureStill(session, config)`, the composited still at its own resolution or a requested one, decoupled from the preview swap chain | Swift today; other SDKs follow |
