...
- [ ] High-resolution capture: full-resolution stills decoupled from the preview and supersampled (rendered larger, box-downsampled) for photo-grade anti-aliased edges are in, both proven through the public ABI (full-sensor size recovered, supersampling deterministic and pixel-different from 1x); tiled composition to break the memory ceiling on very large captures, then HDR and wide-gamut stills, remain
- [ ] Physics: rigid bodies drive lens content - the physics field on model.gltf nodes (box/sphere, static/dynamic), per-lens worlds stepped from frame timestamps, proven deterministic end to end by the physics-drop reference lens (settles bit-stable through the public surface); constraint chains land next (kinematic anchors + distance-constraint chains, an earring-style reference lens proven bit-stable); cloth sheets land next (a cloth field on model.gltf nodes, Jolt soft bodies driving a dynamic mesh, a flag reference lens proven to drape deterministically); and Jolt 5.6 strand hair (a hair field on model.gltf, compute-backed soft strands driven by the head pose, drawn as lines, a reference lens proven to settle deterministically) is in; device targets and the on-device head-driven swing follow
