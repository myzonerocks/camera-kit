...
- [ ] Media: deterministic PNG photo capture and zero-copy video recording shipped (goss_engine_capture_photo + goss_engine_recording_start/stop on the Apple encoder, conformance-proved end to end, Swift wrappers) with libyuv behind adapters/image as the one CPU conversion authority; platform photo formats shipped on Apple (goss_engine_capture_photo_as, JPEG/HEIC through the platform encoders, decode-back proved); the MediaCodec recording backend is built on the encoder input surface (device proof pending hardware); audio + A/V sync and EXIF metadata remain
