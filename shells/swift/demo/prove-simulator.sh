...
# -CKConformance, and checks the reported result. Mirrors demo/prove.ts's
# role for the web shell - a real shell driving the real ABI end to end,
...
DEVICE_NAME="${CK_SIM_DEVICE:-iPhone 17 Pro}"
BUNDLE_ID="kit.camera.demo"
DERIVED_DATA="/tmp/ckdemo-sim-prove"

echo "prove-simulator: building camerakit for the iOS Simulator"
(
  cd ../../..
...
xcodebuild -project CameraKitDemo.xcodeproj -scheme CameraKitDemo -sdk iphonesimulator \
  -destination "${DESTINATION}" -derivedDataPath "${DERIVED_DATA}" build

APP_PATH="${DERIVED_DATA}/Build/Products/Debug-iphonesimulator/CameraKitDemo.app"
...
OUTPUT=$(timeout 60 xcrun simctl launch --console-pty --terminate-running-process "${DEVICE_ID}" "${BUNDLE_ID}" -CKConformance 2>&1 || true)
echo "${OUTPUT}"

if echo "${OUTPUT}" | grep -q "CKCONFORMANCE PROOF"; then
